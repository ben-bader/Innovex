      ...
      <div className="grid grid-cols-8 gap-4 grid-rows-2 max-sm:flex max-sm:flex-col py-12">
        ...
        className="flex flex-col gap-4 col-span-4 row-span-1 bg-white/5 h-80 backdrop-blur-3xl rounded-2xl border border-white/30 lg:p-6 p-4">
              ...
              <div className="flex flex-col gap-4">
                {events.map((event)=>(
                    event.id < 3 ? 
                        <a key={event.id} href={`/main/events/${event.id}`} className="flex items-center py-2 px-3 space-x-2 cursor-pointer rounded-md bg-white/10  backdrop-blur-3xl active:scale-[0.99]">
                            ...
                            <p className="flex items-center gap-[2px] text-gray-300 text-xs " >{event.city} <span className="w-[1px] h-3 bg-gray-300 mx-1"/> {event.date}</p>
                            </span>
                        </a>
                    : ""
                ))}
        ...
        className="flex flex-col gap-4 col-span-4 row-span-1 h-80 bg-white/5 backdrop-blur-3xl rounded-2xl border border-white/30  lg:p-6 p-4 overflow-hidden">
            ...
            <div className="flex items-center">
                <Image src="/Images/woman_looking_up.png" alt="world" width={350} height={10} className="absolute lg:left-10 -z-10 bottom-[-0px] drop-shadow-sky-200"/>
            </div>
        </motion.div>
      </div>
    </section>
